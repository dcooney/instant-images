...
var licenseForms = document.querySelectorAll('.license-settings form.license-settings--item');
if (licenseForms) {
  // Loop each formt o add event listener.
  licenseForms.forEach(function (form) {
    form.addEventListener('submit', function (e) {
      ...
      var type = form.classList.contains('valid') ? 'deactivate' : 'activate';
      if (!license) {
        input.focus();
        return;
      }
...
function updateLicense(params) {
  var _instant_img_admin_lo = instant_img_admin_localize,
    root = _instant_img_admin_lo.root,
    nonce = _instant_img_admin_lo.nonce; // eslint-disable-line no-undef
  var api = root + 'instant-images/license/';
  var config = {
    headers: {
      'X-WP-Nonce': nonce,
  ...
  axios__WEBPACK_IMPORTED_MODULE_1__["default"].post(api, JSON.stringify(params), config).then(function (data) {
    if (data !== null && data !== void 0 && data.data) {
      location.reload(); // Success: reload window.
    } else {
      console.warn(
      // Error handling.
      (0,_wordpress_i18n__WEBPACK_IMPORTED_MODULE_0__.__)('Are you sure you should be doing this?', 'instant-images'));
      doReload();
    }
...
/*!*******************************!*\
  ...
  \*******************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _licenses__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./licenses */ "./admin/src/js/licenses.js");
...
jQuery(document).ready(function ($) {
  ...
  if (wp.media) {
    var _frame = $('#instant_images_modal');
    if (_frame.length) {
      instant_images.setEditor(_frame);
    }
  }
  ...
  var settings = document.querySelectorAll('.settings_page_instant-images-settings .settings-entry');
  if (settings) {
    var hash = window.location.hash;
    if (hash) {
      scrollToSection(hash);
    }
    var nav = document.querySelector('.settings_page_instant-images-settings nav.jump-nav');
