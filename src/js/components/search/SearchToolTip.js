...
export default function SearchToolTip() {
	...
	return (
		<div className={classNames('control-nav--search-tooltip', search?.active ? null : 'hide')}>
			<span title={`${search?.results} ${instant_img_localize.search_results} ${search?.term}`}>{search?.results}</span>
			<button type="button" title={instant_img_localize.clear_search} onClick={() => getPhotos(true)}>
